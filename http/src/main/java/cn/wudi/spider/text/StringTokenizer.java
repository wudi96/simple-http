 ...
 * method is much simpler than the one used by the <code>StreamTokenizer</code> class. The
 ...
 * <p>
 * An instance of <code>StringTokenizer</code> behaves in one of two ways, depending on whether it
 * was created with the
 * <code>returnDelims</code> flag having the value <code>true</code>
 * or <code>false</code>:
 * <ul>
 * <li>If the flag is <code>false</code>, delimiter characters serve to
 * separate tokens. A token is a maximal sequence of consecutive characters that are not
 * delimiters.
 * <li>If the flag is <code>true</code>, delimiter characters are themselves
 * considered to be tokens. A token is thus either one delimiter character, or a maximal sequence of
 * consecutive characters that are not delimiters.
 * </ul><p>
 * A <tt>StringTokenizer</tt> object internally maintains a current position within the string to be
 * tokenized. Some operations advance this current position past the characters processed.<p> A
 * token is returned by taking a substring of the string that was used to create the
 * <tt>StringTokenizer</tt> object.
 * <p>
 ...
 * compatibility reasons although its use is discouraged in new code. It is recommended that anyone
 * seeking this functionality use the <tt>split</tt> method of <tt>String</tt> or the
 * java.util.regex package instead.
 * <p>
 * The following example illustrates how the <tt>String.split</tt> method can be used to break up a
...
public class StringTokenizer implements Enumeration<Object> {
   ...
   * <code>delim</code> argument are the delimiters for separating tokens.
   * <p>
   * If the <code>returnDelims</code> flag is <code>true</code>, then the delimiter characters are
   * also returned as tokens. Each delimiter is returned as a string of length one. If the flag is
   ...
   * <tt>true</tt>, then a subsequent call to
   ...
   * considered to be delimiters by this
   * <tt>StringTokenizer</tt> object is changed to be the characters in
   * the string <tt>delim</tt>. Then the next token in the string after the current position is
   * returned. The current position is advanced beyond the recognized token.  The new delimiter set
   ...
   * value is <code>Object</code> rather than
